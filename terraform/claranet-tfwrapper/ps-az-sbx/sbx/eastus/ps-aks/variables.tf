...
variable "simple_storage_accounts" {
  description = "Simple storage accounts (bypasses Claranet module security checks)"
  type = map(object({
...
variable "container_registries" {
  ...
  type = map(object({
    ...
    anonymous_pull_enabled     = optional(bool, false)     # Allow anonymous pulls
    data_endpoint_enabled      = optional(bool, false)     # Enable data endpoint (Premium only)
