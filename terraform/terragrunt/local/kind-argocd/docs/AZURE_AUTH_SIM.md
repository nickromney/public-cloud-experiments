...
## HTTPS (required for PKCE / crypto.subtle)

When using `*.sslip.io` hostnames (e.g. `dev.127.0.0.1.sslip.io`), browsers do **not** treat `http://...` as a secure context.
OIDC PKCE requires `crypto.subtle`, so the protected React SPA must be served over **HTTPS**.

This stack uses:

- **mkcert** to create a locally trusted CA on your laptop
- **cert-manager** in the cluster
- a **ClusterIssuer** (`mkcert-ca`) backed by a Secret created from your mkcert CA
...
### One-time cluster setup (after stage 700)

```bash
./terraform/terragrunt/local/kind-argocd/scripts/bootstrap-mkcert-ca.sh
```

Verify certs were issued:

```bash
kubectl -n azure-auth-gateway get certificate azure-auth-gateway-tls
```

Then access:

- `https://subnetcalc.dev.127.0.0.1.sslip.io/`
- `https://subnetcalc.uat.127.0.0.1.sslip.io/`
...
- **Workload deployment**: Argo CD tracks the `policies` repo and applies the `apps/azure-auth-sim/` overlays (dev/uat), plus the shared `apps/azure-auth-gateway/`. The kustomize set deploys Keycloak, OAuth2 Proxy, APIM simulator, FastAPI backend, and protected React frontend using registry images `host.docker.internal:3000/gitea-admin/azure-auth-sim-*`. Terraform creates `gitea-registry-creds` in the workload namespaces so the pulls succeed. NGINX Gateway Fabric (see `apps/nginx-gateway-fabric`) exposes the entrypoint via NodePort `30070` mapped to host `443`.
...
- Local default (HTTPS): `https://subnetcalc.dev.127.0.0.1.sslip.io/` and `https://subnetcalc.uat.127.0.0.1.sslip.io/`.
- Optional local vanity host: add `/etc/hosts` entry `127.0.0.1 azure-auth.local` and set Gateway/OAuth2/SPA URLs to `https://azure-auth.local/`.
...
- After stage 900: check Argo CD app status (`kubectl -n argocd get applications`), ensure registry secret `gitea-registry-creds` exists in `azure-auth-sim`, and hit the gateway entry (`https://subnetcalc.dev.127.0.0.1.sslip.io/`) to confirm the OAuth2 proxy/front end flow (port-forward `svc/<name>` if you need to reach Keycloak, APIM, or the API directly).
