...
class TestValidateEndpoint:
    """Tests for /subnets/validate endpoint."""

    def test_valid_ipv4_address(self, client):
        """Test validation of valid IPv4 address."""
        response = client.post("/subnets/validate", json={"address": "192.168.1.100"})
    ...
    def test_valid_ipv4_network(self, client):
        ...
        response = client.post(
            "/subnets/validate", json={"address": "192.168.1.0/24"}
        )
        assert response.status_code == 200
        data = response.json()
        assert data["valid"] is True
    ...
    def test_valid_ipv6_address(self, client):
        ...
        response = client.post(
            "/subnets/validate", json={"address": "2001:db8::1"}
        )
        assert response.status_code == 200
        data = response.json()
        assert data["valid"] is True
    ...
    def test_invalid_address(self, client):
        ...
        response = client.post("/subnets/validate", json={"address": "not-an-ip"})
        assert response.status_code == 400

    def test_missing_address_field(self, client):
        """Test validation without address field."""
        response = client.post("/subnets/validate", json={})
        assert response.status_code == 422


class TestCheckPrivate:
    """Tests for /subnets/check-private endpoint."""

    def test_rfc1918_10_network(self, client):
        """Test RFC1918 10.0.0.0/8 detection."""
        response = client.post(
            "/subnets/check-private", json={"address": "10.50.100.200"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_rfc1918_172_network(self, client):
        ...
        response = client.post(
            "/subnets/check-private", json={"address": "172.20.1.1"}
        )
        assert response.status_code == 200
        data = response.json()
        assert data["is_rfc1918"] is True
    ...
    def test_rfc1918_192_network(self, client):
        ...
        response = client.post(
            "/subnets/check-private", json={"address": "192.168.100.1"}
        )
        assert response.status_code == 200
        data = response.json()
        assert data["is_rfc1918"] is True
    ...
    def test_rfc6598_shared_address_space(self, client):
        ...
        response = client.post(
            "/subnets/check-private", json={"address": "100.65.1.1"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_public_ipv4_address(self, client):
        ...
        response = client.post("/subnets/check-private", json={"address": "8.8.8.8"})
        assert response.status_code == 200
        data = response.json()
    ...
    def test_ipv6_rejected(self, client):
        ...
        response = client.post(
            "/subnets/check-private", json={"address": "2001:db8::1"}
        )
        assert response.status_code == 400


class TestCheckCloudflare:
    """Tests for /subnets/check-cloudflare endpoint."""

    def test_cloudflare_ipv4_address(self, client):
        """Test Cloudflare IPv4 range detection."""
        response = client.post(
            "/subnets/check-cloudflare", json={"address": "104.16.0.1"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_cloudflare_ipv6_address(self, client):
        ...
        response = client.post(
            "/subnets/check-cloudflare", json={"address": "2606:4700::1"}
        )
        assert response.status_code == 200
        data = response.json()
        assert data["is_cloudflare"] is True
    ...
    def test_non_cloudflare_ipv4(self, client):
        ...
        response = client.post(
            "/subnets/check-cloudflare", json={"address": "8.8.8.8"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_cloudflare_ipv4_network(self, client):
        ...
        response = client.post(
            "/subnets/check-cloudflare", json={"address": "104.16.0.0/13"}
        )
        assert response.status_code == 200
        data = response.json()
        assert data["is_cloudflare"] is True
    ...
    def test_invalid_address_format(self, client):
        ...
        response = client.post(
            "/subnets/check-cloudflare", json={"address": "not-an-ip"}
        )
        assert response.status_code == 400


class TestIPv4SubnetCalculation:
    """Tests for /subnets/ipv4 endpoint."""
    ...
    def test_standard_subnet_aws_mode(self, client):
        ...
        response = client.post(
            "/subnets/ipv4", json={"network": "10.0.1.0/24", "mode": "AWS"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_standard_subnet_oci_mode(self, client):
        ...
        response = client.post(
            "/subnets/ipv4", json={"network": "172.16.0.0/24", "mode": "OCI"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_standard_subnet_standard_mode(self, client):
        ...
        response = client.post(
            "/subnets/ipv4", json={"network": "10.0.0.0/24", "mode": "Standard"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_slash_31_subnet(self, client):
        ...
        response = client.post(
            "/subnets/ipv4", json={"network": "10.0.0.0/31", "mode": "Standard"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_slash_32_subnet(self, client):
        """Test /32 host route."""
        response = client.post(
            "/subnets/ipv4", json={"network": "10.0.0.1/32", "mode": "Standard"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_large_subnet(self, client):
        """Test large /8 subnet."""
        response = client.post(
            "/subnets/ipv4", json={"network": "10.0.0.0/8", "mode": "Azure"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_invalid_mode(self, client):
        ...
        response = client.post(
            "/subnets/ipv4", json={"network": "192.168.1.0/24", "mode": "InvalidMode"}
        )
        assert response.status_code == 400

    def test_missing_network_field(self, client):
        """Test missing network field."""
        response = client.post("/subnets/ipv4", json={"mode": "Azure"})
        assert response.status_code == 422

    def test_ipv6_rejected(self, client):
        """Test that IPv6 networks are rejected."""
        response = client.post(
            "/subnets/ipv4", json={"network": "2001:db8::/64", "mode": "Azure"}
        )
        assert response.status_code == 400
    ...
    def test_wildcard_mask(self, client):
        ...
        response = client.post(
            "/subnets/ipv4", json={"network": "192.168.1.0/24", "mode": "Standard"}
        )
        assert response.status_code == 200
        data = response.json()
...
class TestIPv6SubnetCalculation:
    """Tests for /subnets/ipv6 endpoint."""

    def test_standard_ipv6_subnet(self, client):
        """Test IPv6 subnet calculation."""
        response = client.post("/subnets/ipv6", json={"network": "2001:db8::/64"})
        assert response.status_code == 200
        data = response.json()
    ...
    def test_ipv4_rejected(self, client):
        ...
        response = client.post("/subnets/ipv6", json={"network": "192.168.1.0/24"})
        assert response.status_code == 400
