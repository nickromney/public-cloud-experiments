#!/usr/bin/env bash
#
# Azure CLI and SWA CLI mocking functions for BATS tests
# Provides mock implementations without requiring Azure login

# File to track az and swa commands across subshells
export AZ_CALLS_FILE="${BATS_TEST_TMPDIR:-/tmp}/az_calls_$$"
export SWA_CALLS_FILE="${BATS_TEST_TMPDIR:-/tmp}/swa_calls_$$"

# Track az commands called during tests
declare -a AZ_COMMANDS_CALLED=()
declare -a AZ_ARGS_CALLED=()

# Track swa commands called during tests
declare -a SWA_COMMANDS_CALLED=()
declare -a SWA_ARGS_CALLED=()
...
# Capture swa command and arguments
capture_swa_call() {
  local full_command="$*"
  # Write to file for cross-process tracking
  echo "$full_command" >> "$SWA_CALLS_FILE"
  # Also track in arrays for current process
  SWA_COMMANDS_CALLED+=("$full_command")
  SWA_ARGS_CALLED+=("$*")
}

# Note: We use PATH-based mock scripts (tests/bin/az, tests/bin/swa) instead of bash functions
# This ensures the mocks work in subprocesses that scripts spawn
...
mock_swa_setup() {
  reset_swa_tracking
  export SWA_MOCK_DEPLOYMENT_FAILED="false"
}
...
# Mock SWA deployment failure
mock_swa_deployment_failed() {
  export SWA_MOCK_DEPLOYMENT_FAILED="true"
}
...
# Assertions for swa commands

# Assert swa was called with specific command
assert_swa_called() {
  local expected="$1"

  # Read calls from file (for cross-process tracking)
  if [[ -f "$SWA_CALLS_FILE" ]]; then
    if grep -q "$expected" "$SWA_CALLS_FILE"; then
      return 0
    fi
  fi

  # Also check in-process array as fallback
  for cmd in "${SWA_COMMANDS_CALLED[@]}"; do
    if [[ "$cmd" =~ $expected ]]; then
      return 0
    fi
  done

  echo "Expected swa to be called with: $expected" >&2
  echo "Actual calls:" >&2
  if [[ -f "$SWA_CALLS_FILE" ]]; then
    cat "$SWA_CALLS_FILE" >&2
  else
    printf '%s\n' "${SWA_COMMANDS_CALLED[@]}" >&2
  fi
  return 1
}

# Assert swa was called with specific argument
assert_swa_arg() {
  local arg_name="$1"
  local arg_value="$2"
  ...
  echo "Expected swa to be called with: $arg_name $arg_value" >&2
  echo "Actual calls:" >&2
  if [[ -f "$SWA_CALLS_FILE" ]]; then
    cat "$SWA_CALLS_FILE" >&2
  else
...
# Get number of times swa was called
swa_call_count() {
  if [[ -f "$SWA_CALLS_FILE" ]]; then
    wc -l < "$SWA_CALLS_FILE" | tr -d ' '
  else
    echo "${#SWA_COMMANDS_CALLED[@]}"
  fi
}

# Print all swa commands called (for debugging)
print_swa_calls() {
  echo "=== SWA commands called ===" >&2
  if [[ -f "$SWA_CALLS_FILE" ]]; then
    cat "$SWA_CALLS_FILE" >&2
  else
    printf '%s\n' "${SWA_COMMANDS_CALLED[@]}" >&2
