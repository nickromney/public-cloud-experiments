...
# Assert script has required log functions
assert_has_log_functions() {
  local script="$1"
  local -a required_functions=("log_info" "log_error" "log_step" "log_warn")

  for func in "${required_functions[@]}"; do
    if ! grep -q "^${func}()" "$script"; then
      echo "Script $script should define function: $func" >&2
      return 1
    fi
  done
}
