#!/usr/bin/env bash
#
# Create Azure network with Container Instance delegated subnets
# - VNET: 10.0.0.0/16
# - Subnet 1: 10.0.10.0/24 (delegated to Microsoft.ContainerInstance/containerGroups)
# - Subnet 2: 10.0.20.0/24 (delegated to Microsoft.ContainerInstance/containerGroups, private)
...
readonly DELEGATION="Microsoft.ContainerInstance/containerGroups"
...
# Create Subnet 1 (delegated to ACI, public)
if ! az network vnet subnet show --name "${SUBNET1_NAME}" --vnet-name "${VNET_NAME}" --resource-group "${RESOURCE_GROUP}" &>/dev/null; then
  log_info "Creating subnet ${SUBNET1_NAME} (delegated to Container Instances)"
  "${SCRIPT_DIR}/resource-subnet.sh" "${RESOURCE_GROUP}" "${VNET_NAME}" "${SUBNET1_NAME}" "${SUBNET1_PREFIX}" "${NSG_NAME}" "false" "${DELEGATION}"
else
...
log_info ""
log_info "Done! Network created successfully with Container Instance delegations"
